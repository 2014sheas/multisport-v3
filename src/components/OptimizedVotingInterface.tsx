...
import { Heart, ArrowRight, Trash2, X, Vote, RefreshCw } from "lucide-react";
...
export default function OptimizedVotingInterface({
  ...
  const fetchInitialPlayers = useCallback(async () => {
    try {
      ...
      if (response.ok && data.players) {
        ...
        // Pre-fetch next set immediately
        await preFetchNextVoteSet(data.players.map((p) => p.id));
      }
    } catch (error) {
  ...
  const handleEventChange = useCallback(
    async (newEventId: string) => {
      ...
      // Fetch new players for the selected event
      try {
        ...
        if (response.ok && data.players) {
          setCurrentPlayers(data.players);
          // Pre-fetch next set for this event
          await preFetchNextVoteSet(data.players.map((p) => p.id));
        }
      } catch (error) {
  ...
  }, [votingEventId, fetchInitialPlayers]);
  ...
  // Submit vote with instant transition
  const submitVote = useCallback(async () => {
    ...
    try {
      ...
      // Instant transition to next vote set
      setIsTransitioning(true);

      // Use pre-fetched players for instant transition
      if (nextPlayers.length > 0) {
        setCurrentPlayers(nextPlayers);
        setVoteSelections({ keep: null, trade: null, cut: null });

        // Pre-fetch the next set after this one
        setTimeout(() => {
          preFetchNextVoteSet(nextPlayers.map((p) => p.id));
        }, 100);
      } else {
        // Fallback: fetch new players if pre-fetch failed
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      ...
      >
          ...
          {!selectedEventId && votingEventId && (
            <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-md">
              <p className="text-xs text-blue-700">
                🎲 <strong>Randomly selected:</strong> {getSelectedEventName()}
            ...
            return (
              ...
              >
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    onClick={() => handlePlayerSelection(player.id, "trade")}
                    className={`flex flex-col items-center justify-center p-2 rounded text-xs font-medium transition-all duration-200 ${
                      isTrade
        ...
        <div className="flex space-x-2">
          ...
          <button
            onClick={submitVote}
            disabled={voting || !canSubmitVote || isTransitioning}
